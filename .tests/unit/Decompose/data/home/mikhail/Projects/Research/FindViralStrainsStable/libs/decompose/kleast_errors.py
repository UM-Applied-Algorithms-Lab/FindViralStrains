...
def parse_arguments():
    ...
    requiredNamed.add_argument(
        "-o", "--output", type=str, help="Output base filename", required=True
    )
    requiredNamed.add_argument(
        "-M",
        "--maxpaths",
...
def print_thread_info(threads):
    ...
    if threads == 0:
        print(
            "INFO: Using as many threads as possible (up to 32) for the Gurobi solver"
        )
    else:
    ...
    with open(output_path, "w") as f:
        ...
        for index, path in enumerate(paths["paths"]):
            ...
            if multigraph_decomposer is not None:
                # Check the format of path elements
                if path and isinstance(path[0], tuple):
                    # For multigraphs, the path might be a list of (u, v, key) tuples
                    path_str = " ".join(f"{u}-{v}({key})" for u, v, key in path)
                else:
                    # For regular graphs or simple paths, the path is a list of nodes
...
def draw_labeled_multigraph(G, attr_name, ax=None, decimal_places=2, paths=None):
    ...
    try:
        ...
        print("Warning: sfdp not found, using spring layout instead")
    ...
    if paths and "paths" in paths:
        ...
        for idx, path in enumerate(paths["paths"]):
            # Check path format
            if path and isinstance(path[0], tuple) and len(path[0]) == 3:
                # Path is list of (u, v, key) tuples
                edge_list = path
            else:
...
def generate_output_files(
    ...
    # Set Gurobi parameters globally
    if threads > 0:
        gp.setParam("Threads", threads)
    else:
        gp.setParam("Threads", 0)  # 0 means use all available cores
    ...
    for num_paths in range(max_paths, min_paths - 1, -1):
        ...
        edges_to_ignore = get_all_edges_for_node(graph, "0") + get_all_edges_for_node(
            graph, "1"
        )

        # Perform k-least errors analysis for current number of paths
        k_least = fp.kLeastAbsErrors(
            G=graph,
            k=num_paths,
            flow_attr="flow",
