...
def test_Prune(conda_prefix):

    with tempfile.TemporaryDirectory() as tmpdir:
        workdir = Path(tmpdir) / "workdir"
        config_path = Path(".tests/unit/Prune/config")
        data_path = Path(".tests/unit/Prune/data")
        expected_path = Path(".tests/unit/Prune/expected")
        ...
        check_output(
            [
                ...
                "-m",
                "snakemake",
                "output/path_test/graphs/simulated/pruned.dbg",
                "--snakefile",
