...
def test_Unzip(conda_prefix):

    with tempfile.TemporaryDirectory() as tmpdir:
        workdir = Path(tmpdir) / "workdir"
        config_path = Path(".tests/unit/Unzip/config")
        data_path = Path(".tests/unit/Unzip/data")
        expected_path = Path(".tests/unit/Unzip/expected")
        ...
        check_output(
            [
                ...
                "-m",
                "snakemake",
                "output/path_test/read_data/trimmed/simulated/simulated.merged.fq",
                "--snakefile",
